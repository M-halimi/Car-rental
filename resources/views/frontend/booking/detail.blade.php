...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto">
        <a href="{{ route('frontend.dashboard') }}" class="text-white/55 hover:text-white mb-6 inline-flex items-center gap-2 text-sm transition">
        ...
        <div class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl overflow-hidden">
            <div class="bg-accent/10 p-6">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-white/55 text-sm mt-1">#BK{{ str_pad($booking->id, 6, '0', STR_PAD_LEFT) }}</p>
                    </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        <h3 class="text-sm font-semibold text-white/55 uppercase tracking-wider mb-4">{{ __('frontend.vehicle') }}</h3>
                        <div class="bg-[rgba(255,255,255,0.04)] rounded-xl p-4 space-y-2 text-sm">
                            <p><span class="text-white/55">{{ __('frontend.brand') }}:</span> {{ $booking->vehicle->brand }} {{ $booking->vehicle->model }}</p>
                            <p><span class="text-white/55">{{ __('frontend.year') }}:</span> {{ $booking->vehicle->year }}</p>
                            <p><span class="text-white/55">{{ __('frontend.transmission') }}:</span> {{ __("frontend.{$booking->vehicle->transmission}") }}</p>
                            <p><span class="text-white/55">{{ __('frontend.fuel_type') }}:</span> {{ __("frontend.{$booking->vehicle->fuel_type}") }}</p>
                        </div>
                    </div>
                    <div>
                        <h3 class="text-sm font-semibold text-white/55 uppercase tracking-wider mb-4">{{ __('frontend.booking_summary') }}</h3>
                        <div class="bg-[rgba(255,255,255,0.04)] rounded-xl p-4 space-y-2 text-sm">
                            <p><span class="text-white/55">{{ __('frontend.pickup') }}:</span> {{ $booking->pickup_date->format('M d, Y') }}</p>
                            <p><span class="text-white/55">{{ __('frontend.return') }}:</span> {{ $booking->return_date->format('M d, Y') }}</p>
                            <p><span class="text-white/55">{{ __('frontend.days') }}:</span> {{ $booking->total_days }}</p>
                            <p><span class="text-white/55">{{ __('frontend.total_amount') }}:</span> <span class="font-bold text-accent">{{ number_format($booking->total_amount, 2) }} {{ __('frontend.dh') }}</span></p>
                        </div>
                    </div>
                </div>
                ...
                @if($booking->notes)
                    <div class="mb-8">
                        <h3 class="text-sm font-semibold text-white/55 uppercase tracking-wider mb-4">{{ __('frontend.notes_optional') }}</h3>
                        <div class="bg-[rgba(255,255,255,0.04)] rounded-xl p-4 text-sm">
                            <p class="text-white/70">{{ $booking->notes }}</p>
                        </div>
                    </div>
                @endif

                <div class="mb-8">
                    <h3 class="text-sm font-semibold text-white/55 uppercase tracking-wider mb-4">{{ __('frontend.payment_summary') ?? 'Payment Summary' }}</h3>
                    <div class="bg-[rgba(255,255,255,0.04)] rounded-xl p-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div class="text-center p-3 bg-[rgba(255,255,255,0.04)] rounded-lg">
                                <p class="text-xs text-white/55">{{ __('frontend.amount_paid') ?? 'Amount Paid' }}</p>
                                <p class="text-xl font-bold text-success">{{ number_format($totalPaid, 2) }} {{ __('frontend.dh') }}</p>
                            </div>
                            <div class="text-center p-3 bg-[rgba(255,255,255,0.04)] rounded-lg">
                                <p class="text-xs text-white/55">{{ __('frontend.remaining_balance') ?? 'Remaining Balance' }}</p>
                                <p class="text-xl font-bold {{ $remainingBalance > 0 ? 'text-danger' : 'text-white/55' }}">{{ number_format($remainingBalance, 2) }} {{ __('frontend.dh') }}</p>
                            </div>
                            <div class="text-center p-3 bg-[rgba(255,255,255,0.04)] rounded-lg">
                                <p class="text-xs text-white/55">{{ __('frontend.deposit_status') ?? 'Deposit Status' }}</p>
                                <span class="inline-block px-3 py-1 rounded-lg text-sm font-medium border mt-1
                                    @switch($booking->deposit_status)
                                        ...
                                        @case('waived') bg-white/5 text-white/55 border-[rgba(255,255,255,0.1)] @break
                        ...
                        @if($remainingBalance > 0)
                            <div class="w-full bg-[rgba(255,255,255,0.1)] rounded-full h-2 overflow-hidden">
                                @php
                            ...
                            <div class="flex justify-between text-xs text-white/55 mt-1">
                ...
                @if($payments->count() > 0)
                    <div class="mb-8">
                        <h3 class="text-sm font-semibold text-white/55 uppercase tracking-wider mb-4">{{ __('frontend.payment_history') ?? 'Payment History' }}</h3>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-[rgba(255,255,255,0.04)]">
                                    @foreach($payments as $payment)
                                        <tr class="hover:bg-[rgba(255,255,255,0.03)] transition">
                                            ...
                                            <td class="py-3 px-2 text-white/55">{{ ucfirst(str_replace('_', ' ', $payment->payment_method)) }}</td>
                                            <td class="py-3 px-2 text-white/55">{{ ucfirst($payment->payment_type) }}</td>
                                            <td class="py-3 px-2">
                                                <span class="inline-block px-2 py-0.5 rounded-lg text-xs font-medium border
                                                    @switch($payment->status)
                                                        ...
                                                        @case('pending') bg-white/5 text-white/55 border-[rgba(255,255,255,0.1)] @break
                                                        @case('refunded') bg-blue-500/10 text-accent border-accent/20 @break
                ...
                <div class="flex gap-4">
                    <a href="{{ route('frontend.dashboard') }}"
                        class="flex-1 bg-[rgba(255,255,255,0.06)] hover:bg-[rgba(255,255,255,0.1)] text-white/70 hover:text-white py-3 rounded-lg text-center font-medium transition text-sm">
