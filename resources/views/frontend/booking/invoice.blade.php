...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl overflow-hidden">
            <div class="border-b border-[rgba(255,255,255,0.06)] p-8">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-white/55 mt-1 text-sm">{{ __('frontend.invoice_number') }}{{ str_pad($booking->id, 6, '0', STR_PAD_LEFT) }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-white/55 text-sm">{{ __('frontend.invoice_date') }}: {{ $booking->created_at->format('M d, Y') }}</p>
            ...
            <div class="p-8">
                <div class="grid grid-cols-2 gap-8 mb-8">
                    <div>
                        <h3 class="text-xs font-semibold text-white/55 uppercase tracking-wider mb-2">{{ __('frontend.customer') ?? 'Customer' }}</h3>
                        <p class="font-medium">{{ $booking->customer->first_name }} {{ $booking->customer->last_name }}</p>
                        <p class="text-white/55 text-sm">{{ $booking->customer->phone }}</p>
                    </div>
                    <div class="text-right">
                        <h3 class="text-xs font-semibold text-white/55 uppercase tracking-wider mb-2">{{ __('frontend.vehicle') }}</h3>
                        <p class="font-medium">{{ $booking->vehicle->brand }} {{ $booking->vehicle->model }}</p>
                        <p class="text-white/55 text-sm">{{ $booking->vehicle->year }} - {{ $booking->vehicle->registration_number }}</p>
                    </div>
                </div>

                <table class="w-full mb-8">
                    <thead>
                        <tr class="border-b-2 border-[rgba(255,255,255,0.1)]">
                            <th class="text-left py-3 text-xs font-semibold text-white/55 uppercase">{{ __('frontend.description') ?? 'Description' }}</th>
                            <th class="text-right py-3 text-xs font-semibold text-white/55 uppercase">{{ __('frontend.days') }}</th>
                            <th class="text-right py-3 text-xs font-semibold text-white/55 uppercase">{{ __('frontend.total_amount') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                <div class="border-t border-[rgba(255,255,255,0.06)] pt-8 text-center text-white/40 text-sm">
                    <p>DriveNow - {{ __('frontend.hero_subtitle') }}</p>
                    <p class="mt-1">contact@carrental.ma | +212 522 123 456</p>
                </div>
            </div>

            <div class="bg-[rgba(255,255,255,0.03)] p-6 text-center">
