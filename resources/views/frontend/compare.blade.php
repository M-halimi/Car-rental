...
<div class="container mx-auto px-4 py-8">
    ...
    @else
        <div class="overflow-x-auto">
            <table class="w-full bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl overflow-hidden">
                <thead>
                    <tr class="bg-accent/10">
                        <th class="p-4 text-left text-white/55 text-sm font-medium">{{ __('frontend.specification') }}</th>
                ...
                <tbody>
                    <tr class="border-t border-[rgba(255,255,255,0.06)]">
                        <td class="p-4 text-white/55 font-medium">{{ __('frontend.image') }}</td>
                        @foreach($vehicles as $vehicle)
                            <td class="p-4 text-center">
                                <div class="h-24 w-32 mx-auto rounded-lg overflow-hidden bg-[rgba(255,255,255,0.04)] flex items-center justify-center text-4xl">
                                    @php
                    ...
                    <tr class="border-t border-[rgba(255,255,255,0.06)]">
                        <td class="p-4 text-white/55 font-medium">{{ __('frontend.daily_rate') }}</td>
                        @foreach($vehicles as $vehicle)
                            <td class="p-4 text-center text-accent font-bold text-xl">{{ $vehicle->daily_rate }} {{ __('frontend.dh') }}{{ __('frontend.per_day') }}</td>
                        @endforeach
                    </tr>
                    <tr class="border-t border-[rgba(255,255,255,0.06)]">
                        <td class="p-4 text-white/55 font-medium">{{ __('frontend.transmission') }}</td>
                        @foreach($vehicles as $vehicle)
                            <td class="p-4 text-center">{{ __("frontend.{$vehicle->transmission}") }}</td>
                        @endforeach
                    </tr>
                    <tr class="border-t border-[rgba(255,255,255,0.06)]">
                        <td class="p-4 text-white/55 font-medium">{{ __('frontend.fuel_type') }}</td>
                        @foreach($vehicles as $vehicle)
                            <td class="p-4 text-center">{{ __("frontend.{$vehicle->fuel_type}") }}</td>
                        @endforeach
                    </tr>
                    <tr class="border-t border-[rgba(255,255,255,0.06)]">
                        <td class="p-4 text-white/55 font-medium">{{ __('frontend.seats') }}</td>
                        @foreach($vehicles as $vehicle)
                            <td class="p-4 text-center">{{ $vehicle->seats }}</td>
                        @endforeach
                    </tr>
                    <tr class="border-t border-[rgba(255,255,255,0.06)]">
                        <td class="p-4 text-white/55 font-medium">{{ __('frontend.year') }}</td>
                        @foreach($vehicles as $vehicle)
                            <td class="p-4 text-center">{{ $vehicle->year }}</td>
                        @endforeach
                    </tr>
                    <tr class="border-t border-[rgba(255,255,255,0.06)]">
                        <td class="p-4 text-white/55 font-medium">{{ __('frontend.features') }}</td>
                        @foreach($vehicles as $vehicle)
                            <td class="p-4 text-center">
