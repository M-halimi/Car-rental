...
<div class="relative overflow-hidden">
    ...
    <section class="container mx-auto px-4 pt-20 pb-16">
        <div class="text-center max-w-4xl mx-auto">
            <div
                x-data="typewriter()"
                x-init="init()"
                class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 min-h-[4rem]"
            >
                {{ __('frontend.hero_title') }}<br>
                <span class="text-accent" x-text="currentText"></span>
                <span class="animate-blink text-accent">|</span>
            </div>
            <p class="text-white/55 text-lg max-w-2xl mx-auto mb-10">{{ __('frontend.hero_subtitle') }}</p>

            <form action="{{ route('frontend.vehicles') }}" method="GET" class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-2xl p-6 max-w-3xl mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        <label class="block text-white/55 text-xs font-semibold mb-1.5">{{ __('frontend.pickup_location') }}</label>
                        <select name="city_id" class="w-full bg-dark border border-[rgba(255,255,255,0.1)] text-white rounded-lg p-3 text-sm focus:outline-none focus:border-accent transition">
                            <option value="" class="bg-dark">{{ __('frontend.select_city') }}</option>
                    ...
                    <div>
                        <label class="block text-white/55 text-xs font-semibold mb-1.5">{{ __('frontend.pickup_date') }}</label>
                        <input type="date" name="pickup_date" class="w-full bg-dark border border-[rgba(255,255,255,0.1)] text-white rounded-lg p-3 text-sm focus:outline-none focus:border-accent transition" min="{{ date('Y-m-d') }}">
                    </div>
                    <div>
                        <label class="block text-white/55 text-xs font-semibold mb-1.5">{{ __('frontend.return_date') }}</label>
                        <input type="date" name="return_date" class="w-full bg-dark border border-[rgba(255,255,255,0.1)] text-white rounded-lg p-3 text-sm focus:outline-none focus:border-accent transition" min="{{ date('Y-m-d') }}">
                    </div>
    ...
    <section class="border-y border-[rgba(255,255,255,0.06)] py-8">
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="text-center">
                    <div class="text-3xl font-bold text-accent">500+</div>
                    <div class="text-white/55 text-sm mt-1">{{ __('frontend.stats_cars') ?? 'Cars' }}</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-accent">12</div>
                    <div class="text-white/55 text-sm mt-1">{{ __('frontend.stats_locations') ?? 'Locations' }}</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-accent">24/7</div>
                    <div class="text-white/55 text-sm mt-1">{{ __('frontend.stats_support') ?? 'Support' }}</div>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            @foreach($featured as $car)
                <div class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl overflow-hidden hover:border-accent/30 transition group">
                    <div class="h-48 flex items-center justify-center text-6xl bg-[rgba(255,255,255,0.03)] group-hover:bg-[rgba(255,255,255,0.06)] transition">
                        {{ $car['image'] }}
                    </div>
                    <div class="p-5">
                        <h3 class="text-lg font-bold">{{ $car['brand'] }} {{ $car['model'] }}</h3>
                        <p class="text-white/55 text-sm mt-1">{{ $car['transmission'] }} &bull; {{ $car['seats'] }} {{ __('frontend.seats') }} &bull; {{ $car['fuel'] }}</p>
                        <div class="mt-4 flex justify-between items-center">
                            <div>
                                <span class="text-2xl font-bold text-accent">{{ $car['price'] }}</span>
                                <span class="text-white/55 text-sm">{{ __('frontend.per_day') }}</span>
                            </div>
    ...
    <section class="border-t border-[rgba(255,255,255,0.06)] py-16">
        <div class="container mx-auto px-4">
            <h2 class="text-2xl font-bold text-center mb-12">{{ __('frontend.why_choose_us') ?? 'Why Choose Us' }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl p-6 text-center">
                    ...
                    <p class="text-white/55 text-sm">{{ __('frontend.feature_best_prices_desc') ?? 'We offer competitive rates with no hidden fees' }}</p>
                </div>
                <div class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl p-6 text-center">
                    <div class="w-12 h-12 bg-accent/10 rounded-lg flex items-center justify-center mx-auto mb-4">
                    ...
                    <p class="text-white/55 text-sm">{{ __('frontend.feature_support_desc') ?? 'Our team is always ready to help you' }}</p>
                </div>
                <div class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl p-6 text-center">
                    <div class="w-12 h-12 bg-accent/10 rounded-lg flex items-center justify-center mx-auto mb-4">
                    ...
                    <p class="text-white/55 text-sm">{{ __('frontend.feature_fleet_desc') ?? 'Well-maintained vehicles for your safety' }}</p>
                </div>
                <div class="bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] rounded-xl p-6 text-center">
                    <div class="w-12 h-12 bg-accent/10 rounded-lg flex items-center justify-center mx-auto mb-4">
                    ...
                    <p class="text-white/55 text-sm">{{ __('frontend.feature_flexible_desc') ?? 'Free cancellation and easy modifications' }}</p>
                </div>
            </div>
        </div>
    </section>
